<?php

namespace App\Http\Controllers\Api\SchoolStaff\V1\SchoolManagement\IncidentReport;

use App\Actions\IncidentReport\GetIncidentReportByIdAction;
use App\Http\Controllers\Controller;
use App\Http\Resources\Api\SchoolStaff\V1\SchoolManagement\IncidentReport\GetIncidentReportResource;

class GetIncidentReportController extends Controller
{
    public function __construct(
        private GetIncidentReportByIdAction $getIncidentReportByIdAction,
    ) {}

    public function __invoke(string $incidentReportId)
    {
        $relationships = [
            'school',
            'incidentType',
        ];

        $incidentReport = $this->getIncidentReportByIdAction->execute($incidentReportId, $relationships);

        $loggedInStaff = auth('school-staff')->user();

        if (is_null($incidentReport) || $incidentReport->school_id !== $loggedInStaff->school_id) {
            return generateErrorApiMessage('Incident report record does not exist', 404);
        }

        $mutatedIncidentReport = new GetIncidentReportResource($incidentReport);

        return generateSuccessApiMessage('Incident report record was retrieved successfully', 200, $mutatedIncidentReport);
    }
}
